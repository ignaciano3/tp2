...
algogram_t *crear_algo(FILE *usuarios){
    ...
    while (getline(&line, &buffer_size, usuarios) != -1) {
        ...
        *pos = i;
        i++;
        hash_guardar(algo->usuarios, line, pos);
    }
...
void ver_siguiente_feed(algogram_t *algo, heap_t *feed) {
    if (!algo->usuario_logueado || heap_esta_vacio(feed)){
        printf("Usuario no loggeado o no hay mas posts para ver");
        return;
    }
    post_t *post = heap_desencolar(feed);
    printf("POST ID %i\n", post->id); //que la clave del post sea afinidad + id
    printf("%s dijo: %s", post->publicador, post->texto);
    printf("Likes: %i", post->cantidad_likes);
}

void crear_proximo_feed(algogram_t *algo, heap_t *feed) {
    hash_iter_t *iter = hash_iter_crear(algo->posts);
    while (!hash_iter_al_final(iter)){
        const char* clave = hash_iter_ver_actual(iter);
        post_t *post = hash_obtener(algo->posts, clave);
        if (post->publicador == algo->usuario_logueado) continue;
        heap_encolar(feed, post);
        hash_iter_avanzar(iter);
    }
}
